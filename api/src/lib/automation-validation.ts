...
export class AutomationValidator {
  ...
  static validateAutomation(automation: Partial<Automation>): ValidationResult {
    ...
    const entityState = this.buildEntitySelectionState(
      automation.trigger,
    ...
    // Validate action
    const actionErrors = this.validateAction(automation.action, entityState);
   ...
   */
  private static buildEntitySelectionState(
    ...
    if (action.type === 'send_campaign') {
      ...
      if (action.config.propertySelection.source === 'manual' && 
          action.config.propertySelection.propertyIds?.length) {
        if (!state.property.selected) {
          state.property = {
            selected: true,
            source: 'action',
   ...
   * Validate entity selection conflicts
   */
  private static validateEntitySelections(
    ...
    if (state.property.selected) {
      ...
      if (state.property.source === 'trigger') {
        const hasPropertyCondition = automation.conditions?.some(
        ...
        if (action.type === 'send_campaign' && 
            action.config.propertySelection.source === 'manual') {
          errors.push({
      ...
      if (trigger.type === 'time_based' && 
          ...
          action.config.campaignType === 'multi_property') {
  ...
  private static generateBestPracticeWarnings(
    ...
    if (automation.trigger?.type === 'time_based' && 
        automation.action?.type === 'send_campaign') {
      
      const hasRecipientFilter = automation.conditions?.some(
        c => c.category === 'buyer_data'
      );
