...
class AutomationTriggerService {
  ...
  private async executeAutomation(automation: any, triggerData: any): Promise<string> {
    ...
    try {
      ...
      const campaignResult = await campaignCreatorService.createCampaignFromAutomation(automation, triggerData, String(execution._id));
  ...
  /**
   * Process property update trigger
   */
  private async processPropertyUpdateTrigger(data: any): Promise<MailivoAutomation[]> {
    const { propertyID, updateType } = data;

    // Find active automations with property_updated trigger
    const automations = await MailivoAutomation.find({
      "trigger.type": "property_updated",
      isActive: true,
    });

    const matchedAutomations: MailivoAutomation[] = [];

    for (const automation of automations) {
      try {
        const triggerConfig = automation.trigger.config as any;
        const automationUpdateType = triggerConfig?.updateType || "any_update";

        // Match updateType
        // 'any_update' matches all update types
        const isMatch = automationUpdateType === "any_update" || automationUpdateType === updateType;

        if (isMatch) {
          // Check if conditions match
          const conditionsMatch = await this.evaluateConditions(automation.conditions, data);
        ...
        logger.error(`Failed to evaluate automation ${automation._id}`, { error });
      }
    }
