...
export class AutomationExecutor {
  ...
  private async executeAction(
    ...
    } else if (config.propertySelection.source === 'trigger' || 
               config.propertySelection.source === 'condition') {
    ...
    const campaignData = {
      ...
      multiPropertyMeta: config.campaignType === 'multi_property' ? {
        type: 'multi-property',
  ...
  private async fetchProperties(propertyIds: string[], userId: string): Promise<any[]> {
    try {
      ...
      const properties = await Promise.all(
        propertyIds.map(id => 
          axios.get(`${this.LANDIVO_API_URL}/property/${id}`)
            .then(res => res.data)
            .catch(err => {
  ...
  private async fetchAndFilterProperties(conditions: any[], userId: string): Promise<any[]> {
    ...
    return allProperties.filter(property => {
      return propertyConditions.every(condition => 
        this.applyFilters(property, condition.conditions, condition.matchAll ?? true)
      );
    });
  }
  ...
  private async fetchAndFilterBuyers(conditions: any[], userId: string): Promise<any[]> {
    ...
    return allBuyers.filter(buyer => {
      return buyerConditions.every(condition => 
        this.applyFilters(buyer, condition.conditions, condition.matchAll ?? true)
      );
    });
  }
