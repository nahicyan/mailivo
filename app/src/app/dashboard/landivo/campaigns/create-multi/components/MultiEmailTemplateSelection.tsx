// app/src/app/dashboard/landivo/campaigns/create-multi/components/MultiEmailTemplateSelection.tsx
"use client";
...
export function MultiEmailTemplateSelection({ formData, setFormData, errors, templates = [], templatesLoading, templatesError }: Props) {
  //  Filter to only show 'multi' type templates
  const multiTemplates = React.useMemo(() => templates.filter((t) => t.type === "multi"), [templates]);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <div className="text-sm text-gray-600">Choose a template for your multi-property campaign email design.</div>
        </CardHeader>
      </Card>
        ...
        <CardContent className="pt-6 space-y-4">
          <div className="space-y-2">
            ...
            <Select
              ...
              disabled={templatesLoading || multiTemplates.length === 0}>
              <SelectTrigger id="email-template" className={errors.emailTemplate ? "border-red-500" : ""}>
                <SelectValue placeholder={templatesLoading ? "Loading templates..." : multiTemplates.length === 0 ? "No templates available" : "Select a template"} />
              </SelectTrigger>
              <SelectContent>
          ...
          {/* Empty State */}
          {!templatesLoading && multiTemplates.length === 0 && (
            <Alert>
              <AlertCircle className="h-4 w-4" />
