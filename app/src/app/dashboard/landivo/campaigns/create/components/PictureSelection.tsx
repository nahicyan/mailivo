...
export function PictureSelection({
  formData,
  setFormData,
  errors,
  selectedTemplate,
  ...
  const renderableComponents = useMemo(() => {
    const maxComponents = Math.min(
      propertyImageComponents.length,
  ...
  const handleImageSelection = (componentId: string, imageIndex: number) => {
    const component = propertyImageComponents.find(
      (c: { id: string; name?: string; order?: number }) => c.id === componentId
    );
    setFormData((prev) => ({
      ...prev,
      imageSelections: {
  ...
  const hasInsufficientImages =
    availableImages.length < propertyImageComponents.length;
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <p className="text-gray-500">
              The selected template doesn't contain any property image
              components. You can skip this step and proceed to audience
              selection.
            </p>
          </div>
        </CardContent>
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <p className="text-red-600">
              This property has no images available. Please select a different
              property or add images to this property.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-gray-600 mt-1">
            Choose which property images to use for each image component in your
            template.
          </p>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center space-x-2 mt-2">
                ...
                <Badge
                  variant={hasInsufficientImages ? "destructive" : "default"}
                >
              ...
              <div className="flex items-start space-x-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-700 mt-1">
                    This property has {availableImages.length} images, but your
                    template has {propertyImageComponents.length} image
                    components. Only the first {availableImages.length}{" "}
          ...
          {renderableComponents.map((component: any, index: number) => {
            const selectedImageIndex =
              imageSelections[component.id]?.imageIndex ??
              component.props?.imageIndex ??
              0;
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="text-xs text-gray-500 space-y-1 p-2 bg-gray-50 rounded">
                    ...
                    <p>
                      ...
                      {imageSelections[component.id]
                        ? `Image ${imageSelections[component.id].imageIndex + 1}`
                        : "Not set"}
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        handleImageSelection(component.id, parseInt(value))
                      }
                    >
                      <SelectTrigger
                        className={
                          errors[`imageSelection_${component.id}`]
                            ? "border-red-500"
                            : ""
                        }
                      >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-gray-500 text-center">
                      Selected: Image {selectedImageIndex + 1} for{" "}
                      {component.name}
                    </p>
                  </div>
                </CardContent>
          ...
          <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
            ...
            <div className="space-y-1 text-sm text-green-700">
              {Object.entries(imageSelections).map(
                ([componentId, selection]: [string, any]) => (
                  <p key={componentId}>
                    • {selection.name} (Order: {selection.order}) → Image{" "}
                    {selection.imageIndex + 1}
                  </p>
                )
              )}
            </div>
