...
export default function TriggerSelector({ value, onChange }: TriggerSelectorProps) {
  ...
  const handleTypeChange = (type: string) => {
    ...
    const defaultConfigs: Record<string, any> = {
      ...
      property_updated: { updateType: 'any' },
            ...
            {value.config.schedule === 'weekly' && (
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(val) => updateConfig({ dayOfWeek: parseInt(val) })}
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue />
                  </SelectTrigger>
            ...
            {value.config.schedule === 'monthly' && (
              <div>
                ...
                <Input
                  ...
                  min="1"
                  max="31"
                  value={value.config.dayOfMonth || 1}
        ...
        return (
          <div className="space-y-4 mt-4 p-4 bg-muted/30 rounded-lg">
            <div className="flex items-center justify-between">
              ...
              <Switch
                checked={value.config.requireLoggedIn}
                onCheckedChange={(checked) => updateConfig({ requireLoggedIn: checked })}
              />
            </div>
        ...
        return (
          <div className="space-y-4 mt-4 p-4 bg-muted/30 rounded-lg">
            <div>
              ...
              <Select 
                value={value.config.updateType || 'any'}
                onValueChange={(val) => updateConfig({ updateType: val })}
              >
                <SelectTrigger className="mt-1">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="any">Any Update</SelectItem>
                  <SelectItem value="price_drop">Price Drop</SelectItem>
                  <SelectItem value="status_change">Status Change</SelectItem>
                  <SelectItem value="availability">Availability Change</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="space-y-4 mt-4 p-4 bg-muted/30 rounded-lg">
            ...
            {value.config.event === 'clicked' && (
              <div>
                ...
                <Input
                  value={value.config.linkText || ''}
                  onChange={(e) => updateConfig({ linkText: e.target.value })}
                  placeholder="Filter by specific link text"
                  className="mt-1"
                />
              </div>
            )}
